 ...
 *
 * @returns {object[]} Sorted array of terms.
 */
 ...
 * @returns {(function(object): (object | boolean))} Matcher function.
 */
 ...
 * @returns {WPElement}        Hierarchical term selector component.
 */
export const TermPicker = ({ slug }) => {
	...
	const { hasCreateAction, hasAssignAction, terms, loading, availableTerms, taxonomy } =
		useSelect(
			(select) => {
				...
				return {
					hasCreateAction: _taxonomy
						? get(
								getCurrentPost(),
								['_links', `wp:action-create-${_taxonomy.rest_base}`],
								false,
						  )
						: false,
	 ...
	 * @returns {Promise} A promise that resolves to save term object.
	 */
	const addTerm = (term) => {
	...
	const filterLabel = get(taxonomy, ['labels', 'search_items'], __('Search Terms'));
